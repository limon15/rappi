...
from common import validar_opcion_ingresada, evaluar_informacion_suficiente, imprimir_aviso_de_retorno_al_menu_anterior, imprimir_aviso_de_retorno_al_menu_anterior, imprimir_titulo_seccion
...
def simulacion_de_pedidos(lista_clientes, lista_restaurantes, lista_rappitenderos):
    ...
    if informacion_suficiente :
        ...
        for i in range(cantidad_simulaciones):   
            ...
            cliente_al_azar_actualizado, rappitendero_al_azar_actualizado = actualizar_ganancias_rappitendero_cliente(importe_total, rappitendero_al_azar, cliente_al_azar)
            restaurante_al_azar_actualizado = actualizar_ventas_restaurante(importe_total, restaurante_al_azar)
        lista_clientes.remove(cliente_al_azar)
        lista_clientes.append(cliente_al_azar_actualizado)
        lista_rappitenderos.remove(rappitendero_al_azar)
        lista_rappitenderos.append(rappitendero_al_azar_actualizado)
        lista_restaurantes.remove(restaurante_al_azar)
        lista_restaurantes.append(restaurante_al_azar_actualizado)
